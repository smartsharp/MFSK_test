...
void olivia::tx_init()
{
	...
	printf("tx_init txbasefreq=%d, mode=%d\n",txbasefreq, mode);
	//rx_flush();
	restart();
{ // critical section
...
void olivia::rx_init()
{
	printf("olivia::rx_init()\n");
	{
		guard_lock dsp_lock(&olivia_mutex);
		Rx->Reset();
		escape = 0;
	}
	printf("olivia::rx_init2()\n");
	restart();
}
...
// whatever there is.
	...
	if (stopflag || (Tx->GetReadReady() < Tx->BitsPerSymbol)) {
		if (!stopflag && (c = get_tx_char()) == GET_TX_CHAR_NODATA/*GET_TX_CHAR_ETX*/)
			stopflag = true;
		if (stopflag) {
			printf("stop tx\n");
			Tx->Stop();
...
{// critical section
	...
	if (Tx->Preset() < 0) {
		printf("olivia: transmitter preset failed!\n");
		return;
	}
	...
	if (Rx->Preset() < 0) {
		printf("olivia: receiver preset failed!\n");
		return;
	}
	fragmentsize = 1024;
...
olivia::olivia(trx_mode omode):
		oliviatones(2),	oliviabw(2),oliviasmargin(8),oliviasinteg(4),olivia_reset_fec(false),
		olivia8bit(true),olivia_start_tones(true)
...
int olivia::getbuflen(){
	return SCBLOCKSIZE; //64;
}
