...
psk::psk(trx_mode pskmode) : modem()
{
	...
	switch (0/*progdefaults.PSK_filter*/) { /* PSK, filter can be 0, 1, 2, 3 or 4 */
		case 1:
...
void psk::rx_symbol(cmplx symbol, int car)
{
	...
	switch (dcdshreg) {
		...
		default:
			if (metric > 5.0/*progStatus.sldrSquelchValue*/ || /*progStatus.*/sqlonoff == false) {
				dcd = true;
			} else {
				dcd = false;
			}
...
int psk::tx_process()
{
	...
	if (preamble > 0) {
		...
		if (_pskr || ((_xpsk || _8psk || _16psk) && !_disablefec) ) {
			...
			printf("preamble=%d, acc_symbols=%d, numcarriers=%d\n", preamble, acc_symbols, numcarriers);
			if (preamble == 0) {
